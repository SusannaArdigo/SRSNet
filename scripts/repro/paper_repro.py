...
SELECTIVITY_VARIANTS = [
    ...
    "srs_paper.SRSNet_TASP",         
    "srs_paper.SRSNet_HypernetAF",   
...
def _selectivity_controls_tasks(scope, gpu, grid, variants=None):
    """Selectivity-controls study: baseline + random controls + extensions.

    grid="small" => 2 datasets x 2 horizons x 5 seeds (default).
    grid="full"  => 4 datasets x 4 horizons x 5 seeds.
    variants=None => SELECTIVITY_VARIANTS (full 9 variants incl. 2/3-axis combos).
    variants=SELECTIVITY_VARIANTS_SIMPLE => only the 5 single-axis variants.
    """
    ...
    variant_list = variants if variants is not None else SELECTIVITY_VARIANTS

    tasks = []
    for dataset in datasets:
        for model_name in variant_list:
...
def build_tasks(scope, gpu):
    ...
    if scope == "selectivity-controls-full-simple":

        tasks.extend(_selectivity_controls_tasks(
            scope, gpu, grid="full", variants=SELECTIVITY_VARIANTS_SIMPLE,
        ))
        return tasks
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "selectivity-controls-full-simple",
